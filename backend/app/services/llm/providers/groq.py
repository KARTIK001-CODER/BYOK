...
logger = logging.getLogger("app.services.llm.providers.groq")


class GroqProvider(LLMProvider):
    """Groq LPU LLM Provider with async HTTP streaming and transient retry support."""
    ...
    def _map_http_error(self, status_code: int, raw_body: str) -> LLMException:
        logger.warning("Groq API error HTTP %d: %s", status_code, raw_body[:300])
        if status_code in (401, 403):
            return LLMException(
                message="The Groq API key is invalid or unauthorized.",
        ...
        if status_code == 400:
            return LLMException(
                message="Invalid request sent to Groq provider.",
                code=LLMErrorCode.LLM_PROVIDER_ERROR,
                status_code=400,
            )
        return LLMException(
            message="Groq provider service encountered an error.",
            code=LLMErrorCode.LLM_PROVIDER_ERROR,
            status_code=503,
        )
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        for attempt in range(self._max_retries + 1):
            try:
                ...
                if attempt < self._max_retries:
                    ...
                    continue
                raise LLMException(
                    message="Groq generation request timed out.",
        ...
        raise LLMException(
            message=f"Groq generation failed: {last_exception}",
            code=LLMErrorCode.GENERATION_FAILED,
        )
    ...
    async def stream(self, request: LLMRequest) -> AsyncGenerator[LLMStreamChunk, None]:
        ...
        try:
            ...
            raise LLMException(
                message="Groq stream connection timed out.",
                code=LLMErrorCode.LLM_TIMEOUT,
                status_code=504,
            ) from exc
        except LLMException:
            ...
            raise LLMException(
                message=f"Groq streaming encountered an error: {e}",
                code=LLMErrorCode.GENERATION_FAILED,
            ) from e
