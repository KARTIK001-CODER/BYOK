...
logger = logging.getLogger("app.services.llm.providers.openai")


class OpenAIProvider(LLMProvider):
    """OpenAI LLM Provider with async HTTP streaming and transient retry support."""
    ...
    def _map_http_error(self, status_code: int, raw_body: str) -> LLMException:
        logger.warning("OpenAI API error HTTP %d: %s", status_code, raw_body[:300])
        ...
        if status_code == 400:
            return LLMException(
                message="Invalid request sent to OpenAI provider.",
                code=LLMErrorCode.LLM_PROVIDER_ERROR,
                status_code=400,
            )
        return LLMException(
            message="OpenAI provider service encountered an error.",
            code=LLMErrorCode.LLM_PROVIDER_ERROR,
            status_code=503,
        )
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        for attempt in range(self._max_retries + 1):
            try:
                async with httpx.AsyncClient(timeout=httpx.Timeout(self._timeout)) as client:
                    ...
                    if not choices:
                        raise LLMException(
                            message="OpenAI returned empty response choices.",
                ...
                if attempt < self._max_retries:
                    ...
                    continue
                raise LLMException(
                    message="OpenAI generation request timed out.",
        ...
        raise LLMException(
            message=f"OpenAI generation failed: {last_exception}",
            code=LLMErrorCode.GENERATION_FAILED,
        )
        ...
        try:
            async with (
                ...
                async for line in response.aiter_lines():
                    ...
                    if not choices:
                        if usage:
                            yield LLMStreamChunk(delta="", finish_reason=None, usage=usage)
                        continue
            ...
            raise LLMException(
                message="OpenAI stream connection timed out.",
                code=LLMErrorCode.LLM_TIMEOUT,
                status_code=504,
            ) from exc
        except LLMException:
            ...
            raise LLMException(
                message=f"OpenAI streaming encountered an error: {e}",
                code=LLMErrorCode.GENERATION_FAILED,
            ) from e
